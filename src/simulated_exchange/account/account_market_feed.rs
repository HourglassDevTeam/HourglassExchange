...
where
    Event: Clone + Send + Sync + 'static, // 约束Event类型必须满足Clone, Send, Sync, 'static特性
{
...
where
    Event: Debug + Clone + Send + Sync + 'static, // 约束Event类型必须满足Debug, Clone, Send, Sync, 'static特性
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // 打印AccountDataStreams的调试信息，包括流的标识符。
        f.debug_struct("AccountMarketStreams")
...
impl<Event> Stream for DataStream<Event>
where
    Event: Clone + Send + Sync + 'static, // 约束Event类型必须满足Clone, Send, Sync, 'static特性
{
    type Item = Event; // 数据流中的元素类型为Event
