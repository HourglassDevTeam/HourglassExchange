...
impl Order<Pending> {
    // 判断是Maker还是Taker单
    pub fn determine_maker_taker(&self, current_price: f64) -> Result<OrderRole,ExecutionError> {
        match self.kind {
            OrderKind::Market => Ok(OrderRole::Taker),
            OrderKind::Limit => match self.side {
                Side::Buy => {
                    // 对于买单，限价单的价格应高于或等于当前价格才为Maker
                    if self.state.price >= current_price {
                        Ok(OrderRole::Maker)
                    } else {
                        Ok(OrderRole::Taker)
                    }
            ...
            OrderKind::PostOnly => match self.side {
                Side::Buy => {
                    ...
                    if self.state.price >= current_price {
                        ...
                        ExecutionError::OrderRejected("PostOnly order rejected".parse().unwrap()).into()
                    }
                }
                Side::Sell => {
                    if self.state.price <= current_price {
            ...
            OrderKind::GoodTilCancelled => match self.side {
                Side::Buy => {
                    // GTC订单和Limit订单相似
                    if self.state.price >= current_price {
                        Ok(OrderRole::Maker)
    ...
    // NOTE 注意size的单位
    pub fn calculate_required_available_balance(&self, current_price: f64,leverage: f64) -> (&Token, f64) {
        match self.kind {
            Instrument::Spot => {
                match self.side {
                    Side::Buy => (&self.instrument.quote, current_price * self.state.size),
                    Side::Sell => (&self.instrument.base, self.state.size),
                }
            }
            Instrument::Perpetual => {
                match self.side {
                    Side::Buy => (&self.instrument.quote, current_price * self.state.size * leverage),
                    Side::Sell => (&self.instrument.base, self.state.size * leverage),
                }
            }
        }
    }
