...
use crate::{
    ...
    sandbox::{utils::chrono_operations::extract_date, ws_trade::parse_base_and_quote},
};
...
impl ClickHouseClient {
    ...
    pub async fn query_unioned_trade_table_batched_between_dates(
        self: Arc<Self>,
        exchange: &str,
        instrument: &str,
        channel: &str,
    ...
    pub async fn optimize_union_tables_in_date_range(&self, exchange: &str, instrument: &str, channel: &str, mut start_date: NaiveDate, end_date: NaiveDate) -> Result<(), Error> {
        ...
        while start_date <= end_date {
            ...
            println!(
                "Date: {} - Union tables processed: {}/{}, Total progress: {:.2}%",
                date_str,
                processed_tables,
                table_names.len(),
                progress
