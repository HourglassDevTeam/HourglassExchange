/// `OrderFills` 的作用在系统中通常是用于跟踪订单的执行状态，尤其是当订单部分或完全被成交时，`OrderFills` 可以记录相关信息，如成交的数量、价格等。这对于分析订单执行的详细情况、生成报告或调试系统中的问题非常有用。
/// 
/// 在你的系统中，如果 `OrderFills` 结构目前没有被使用，可能存在以下几种情况：
/// 
/// ### 1. **未被充分利用**：
/// `OrderFills` 可能是设计时的一个备用结构，用于未来扩展功能或用于更加详细的订单管理。如果系统当前不需要这些详细信息，`OrderFills` 就没有被利用起来。
/// 
/// ### 2. **可能的使用场景**：
