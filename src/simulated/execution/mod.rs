...
pub struct SimulatedClient {
    pub request_tx: mpsc::UnboundedSender<SimulatedCommand>,
}
...
impl ClientExecution for SimulatedClient {
    ...
    async fn fetch_orders_open(&self) -> Result<Vec<Order<Open>>, ExecutionError> {
        ...
        self.request_tx
            .send(SimulatedCommand::FetchOrdersOpen(response_tx))
    ...
    async fn fetch_balances(&self) -> Result<Vec<TokenBalance>, ExecutionError> {
        let (response_tx, response_rx) = oneshot::channel();
        self.request_tx
            .send(SimulatedCommand::FetchBalances(response_tx))
    ...
    async fn open_orders(&self, open_requests: Vec<Order<RequestOpen>>) -> Vec<Result<Order<Open>, ExecutionError>> {
        let (response_tx, response_rx) = oneshot::channel();
        self.request_tx
            .send(SimulatedCommand::OpenOrders((open_requests, response_tx)))
    ...
    async fn cancel_orders(&self, cancel_requests: Vec<Order<RequestCancel>>) -> Vec<Result<Order<Cancelled>, ExecutionError>> {
        let (response_tx, response_rx) = oneshot::channel();
        self.request_tx
            .send(SimulatedCommand::CancelOrders((cancel_requests, response_tx)))
    ...
    async fn cancel_orders_all(&self) -> Result<Vec<Order<Cancelled>>, ExecutionError> {
        let (response_tx, response_rx) = oneshot::channel();
        self.request_tx
            .send(SimulatedCommand::CancelOrdersAll(response_tx))
