...
pub fn fluctuate_latency(latency: &mut AccountLatency, current_time: f64) {
    match latency.fluctuation_mode {
        FluctuationMode::Sine => {
            latency.current_value = ((latency.maximum - latency.minimum) as f64 * (current_time.sin() + 1.0) / 2.0) as i64 + latency.minimum;
        }
        FluctuationMode::Cosine => {
            latency.current_value = ((latency.maximum - latency.minimum) as f64 * (current_time.cos() + 1.0) / 2.0) as i64 + latency.minimum;
        }
        ...
        FluctuationMode::Exponential => {
            let exp_value = ((current_time.exp() % (latency.maximum - latency.minimum) as f64) + latency.minimum as f64) as i64;
            latency.current_value = exp_value.clamp(latency.minimum, latency.maximum);
        }
        FluctuationMode::Logarithmic => {
            let log_value = ((current_time.ln().abs() % (latency.maximum - latency.minimum) as f64) + latency.minimum as f64) as i64;
            latency.current_value = log_value.clamp(latency.minimum, latency.maximum);
        }
        FluctuationMode::LinearIncrease => {
            let linear_value = latency.minimum + (current_time as i64 % (latency.maximum - latency.minimum));
            latency.current_value = linear_value.clamp(latency.minimum, latency.maximum);
        }
        FluctuationMode::LinearDecrease => {
            let linear_value = latency.maximum - (current_time as i64 % (latency.maximum - latency.minimum));
            latency.current_value = linear_value.clamp(latency.minimum, latency.maximum);
        }
        FluctuationMode::StepFunction => {
            let step_size = (latency.maximum - latency.minimum) / 10;
            let step_value = latency.minimum + ((current_time as i64 / step_size) % 10) * step_size;
