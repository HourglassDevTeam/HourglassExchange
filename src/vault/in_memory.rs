...
impl<Statistic> PositionHandler for InMemoryVault<Statistic> {
    fn set_open_position(&mut self, position: Position) -> Result<(), VaultError> {
        let position_id = match &position {
            Position::Perpetual(pos) => pos.meta.position_id.clone(),
            Position::LeveragedToken(pos) => pos.meta.position_id.clone(),
            Position::Future(pos) => pos.meta.position_id.clone(),
            Position::Option(pos) => pos.meta.position_id.clone(),
        };
