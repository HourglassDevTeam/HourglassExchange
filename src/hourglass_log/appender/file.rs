...
//! # 普通文件Appender
//!
//! `FileAppender` 内部使用 `BufWriter` 提高IO性能。
//!
//! ```rust
//! # use hourglass_log::appender::FileAppender;
//! let appender = FileAppender::builder().path("./mylog.log").build();
//! ```
//!
//! # 日志轮换
//! `hourglass_log` 支持在本地时区进行日志轮换。可用的轮换周期如下：
//!
//! - 分钟 `Period::Minute`
//! - 小时 `Period::Hour`
//! - 天 `Period::Day`
...
//!
//! ```rust
//! use hourglass_log::appender::{FileAppender, Period};
//! // 每分钟轮换一次
//! let appender = FileAppender::builder().path("./mylog.log").rotate(Period::Minute).build();
//! ```
//!
//! 当配置为按分钟分割日志文件时，日志文件的名称格式为 `mylog-{MMMM}{YY}{DD}T{hh}{mm}.log`。按天分割时，日志文件名格式为 `mylog-{MMMM}{YY}{DD}.log`。
//!
//! 日志文件名示例：
//! ```sh
//! $ ls
...
//! // 按年
//! current-2023.log
//! // 省略扩展名（例如 "./log"）将把日期时间添加到日志文件名的末尾
//! log-20231026T1353
//! ```
//!
//! ## 日志轮换和自动删除过期日志
//!
//! `hourglass_log` 首先找到由 `hourglass_log` 生成的文件，并根据最后修改时间清理过期日志。`hourglass_log` 根据文件名的文件干和添加的日期时间来找到生成的日志。
//!
//! **注意**：任何匹配模式的文件都将被删除。
//!
//! ```rust
//! use hourglass_log::appender::{Duration, FileAppender, Period};
//! // 清理名为 `current-\d{8}T\d{4}.log` 的文件。
//! // 例如 `another-\d{8}T\d{4}.log` 或 `current-\d{8}T\d{4}` 之类的文件将不会被删除，因为文件名的文件干不匹配。
//! // 像 `current-\d{8}.log` 之类的文件也会保留，因为轮换持续时间不匹配。
//!
//! // 每天轮换一次日志，每次轮换时清理7天前的旧日志
    ...
    fn file<T: AsRef<Path>>(path: T, period: Period, timezone: &LogTimezone) -> PathBuf {
        ...
        if let Some(ext) = p.extension() {
            ...
            p.with_file_name(format!(
                "{}-{}",
                p.file_name().map(|x| x.to_string_lossy()).unwrap_or(Cow::from("log")),
                ts
            ))
        }
    }
                            ...
                            .filter(|x| {
                                ...
                                if let Some((stem, time)) = name.rsplit_once("-") {
                                    ...
                                    len && time.chars().enumerate().all(check)
                                        && path.file_stem().map(|x| x.to_string_lossy() == stem).unwrap_or(false)
                                } else {
                                    false
                                }
                            })
...
mod test {
    ...
    fn to_wait_ms() {
        ...
        let tm = Date::from_calendar_date(2024, Month::January, 1)
            .unwrap()
            .with_hms(0, 0, 0)
            .unwrap()
