...
///
/// # 计算原理
/// 1. **EquityPoint 计算**：每当一个交易头寸结束时（即 `exit_balance` 已经确定），我们就会计算一个 `EquityPoint`，它表示在该时间点的总权益。这包括头寸的结束时间（这里暂时使用当前时间 `Utc::now()`）和总金额。
///
/// 2. **更新回撤 (`update`)**：当一个新的 `EquityPoint` 被生成时，我们使用它来更新当前回撤。如果当前回撤结束（即策略从谷底恢复到新的峰值），我们会更新最大回撤和平均回撤。
...
///
/// # 时间戳的选择
/// `EquityPoint` 的时间戳字段 `time` 目前设置为 `Utc::now()`，即当前时间。这是为了简单起见，如果 `PositionMeta` 中包含了更合适的历史时间戳（如头寸关闭的时间），则应使用该时间戳，以更准确地反映回撤的发生时间。这种方法允许更精确的回撤分析，使得这些数据能够更好地反映策略的历史表现。
///
/// # 用途
