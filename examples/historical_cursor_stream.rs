...
async fn main() {
    ...
    println!("Client created successfully. Attempting to fetch data...");
    ...
    match cursor_result {
        Ok(mut cursor) => {
            ...
            let cursor_task = tokio::spawn(async move {
                loop {
                    match timeout(Duration::from_secs(5), cursor.next()).await {
                        ...
                        Ok(Ok(None)) => {
                            println!("No more data to read from the cursor.");
                            break;
                        }
                        Ok(Err(e)) => {
                            eprintln!("Error while reading cursor: {:?}", e);
                            break;
                        }
                        Err(_) => {
                            eprintln!("Timeout while reading cursor");
                            break;
                        }
                    }
                }
            ...
            if let Err(e) = cursor_task.await {
                eprintln!("Cursor task was aborted: {:?}", e);
            }
